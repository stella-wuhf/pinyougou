...
app.controller('goodsController' ,function($scope,$controller,$location,typeTemplateService ,itemCatService,uploadService ,goodsService){	
	...
	$scope.findOne=function(){
		//$location.search()['id']; 从url中获取携带的参数
	...
	// 查询模板下的品牌列表:  在模板表中查询
	//findOne方法,根据模板id查询模板的对象信息,将返回的对象赋值给$scope.typeTemplate,再获取对象中的brandIds
	//将获取到的brandIds json串转为对象,方便页面中显示
	$scope.$watch("entity.goods.typeTemplateId",function(newValue,oldValue){
		...
		typeTemplateService.findBySpecList(newValue).success(function(response){
			//后台返回的数据是一个List<Map> 单个map中包含id(规格id),text(规格名称),options(规格选项集合)
	...
	$scope.findItemCatList = function(){
		itemCatService.findAll().success(function(response){ //response是List<ItemCat>
			for(var i=0;i<response.length;i++){
				//循环遍历获取到每个itemCat,数组赋值  array[1]=xxx
